...
# 프로젝트 루트를 Python 경로에 추가 (PyInstaller 빌드 시에도 동작, Windows 호환)
...
# config.py를 동적으로 로드 (PyInstaller EXE에서도 작동, Windows 호환)
...
if config_path.exists():
    try:
        ...
        # config 모듈에 기본값 설정 (없는 경우)
        if not hasattr(config, 'LOG_FORMAT'):
            config.LOG_FORMAT = '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        if not hasattr(config, 'LOG_DATE_FORMAT'):
            config.LOG_DATE_FORMAT = '%Y-%m-%d %H:%M:%S'
        if not hasattr(config, 'DEBUG'):
            config.DEBUG = False
        if not hasattr(config, 'LOG_LEVEL'):
            config.LOG_LEVEL = 'INFO'
        if not hasattr(config, 'KIS_APP_KEY'):
            config.KIS_APP_KEY = ''
        if not hasattr(config, 'KIS_APP_SECRET'):
            config.KIS_APP_SECRET = ''
        if not hasattr(config, 'KIS_ACCOUNT_NO'):
            config.KIS_ACCOUNT_NO = ''
        if not hasattr(config, 'US_MARKET_EXCHANGE'):
            config.US_MARKET_EXCHANGE = 'AMS'
        if not hasattr(config, 'US_MARKET_CURRENCY'):
            config.US_MARKET_CURRENCY = 'USD'
        if not hasattr(config, 'US_MARKET_TIMEZONE'):
            config.US_MARKET_TIMEZONE = 'America/New_York'
        if not hasattr(config, 'BALANCE_SYNC_INTERVAL'):
            config.BALANCE_SYNC_INTERVAL = 60
